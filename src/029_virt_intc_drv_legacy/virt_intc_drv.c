...
struct virt_intc_desc {
    ...
    int child_virq_base; // 这些中断资源虚拟中断号的起始地址
...
int virtual_intc_probe(struct platform_device *pdev)
{
    ...
    // 为这些虚拟中断号创建irq_domain，使用irq_domain_add_legacy，并绑定domain_ops
    g_vintc_desc->domain = irq_domain_add_legacy(np,
                                                g_vintc_desc->nirqs,
                                                g_vintc_desc->child_virq_base,
                                                0,
    ...
    DEBUG_LOG("Register irq_domain success: virq=%d, nirqs=%d, virq_base=%d!",
                    g_vintc_desc->virq, g_vintc_desc->nirqs, g_vintc_desc->child_virq_base);

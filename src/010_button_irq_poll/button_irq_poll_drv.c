...
static int push_event(struct button_irq_event *event)
{
    ...
    spin_lock(&btn_event_lock);
     if (cache_event_cnt >= MAX_CACHE_EVENT_COUNT) {
        ...
        btn_events_clist->btn_num = event->btn_num;
    ...
    // Is first event?
    DEBUG_LOG("Insert a new node: KEY%d", ev->btn_num);
    if (!btn_events_clist) {
        btn_events_clist = ev;
...
int pop_event(int btn_num, struct button_irq_event* event)
{
    ...
    DEBUG_LOG("Pop a node: KEY%d", event->btn_num);
    // unlink poped node
    cache_event_cnt--;
...
static int btn_event_match(int btn_num)
{
    struct button_irq_event *tmp = btn_events_clist;
    int count = 0;
    ...
    do {
        if (tmp->btn_num == btn_num)
            count++;
        // else
        //     DEBUG_LOG("Mismatch button num: current=%d, target=%d", tmp->btn_num, btn_num);
...
static __poll_t button_drv_poll(struct file *file, poll_table * wait)
{
    ...
    if (btn_event_match(g_btn_list[minor].btn_num) <= 0) {
...
static void workqueue_handler(struct work_struct *work) {
    DEBUG_LOG("Enter workqueue!");
    if (push_event(&prealloc_event)) {// 在工作队列中安全执行
...
static DECLARE_WORK(button_work, workqueue_handler);
...
static irqreturn_t button_irq_cb(int irqno, void *priv_data)
{
    ...
    prealloc_event.btn_num = btn->btn_num;
    prealloc_event.idx = ++global_irq_idx;
    prealloc_event.val = gpiod_get_value(btn->gpiod);
    prealloc_event.tm  = jiffies;
    // 触发工作队列
...
int stm32mp157_button_probe(struct platform_device *pdev)
{
    ...
    for_each_child_of_node(pn, chid) {
        ...
        // request interrupt
        ret = request_irq(g_btn_list[idx].irq, button_irq_cb, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, g_btn_list[idx].label, &g_btn_list[idx]);
        if (ret) {

...
static struct virtual_pinctrl_pins my_pins[] = {
    ...
    {
        .desc = my_pins_desc[10],
        .mux_flag = PINMUX_I2C_FLAG | PINMUX_GPIO_FLAG,
    },
    {
        .desc = my_pins_desc[11],
        .mux_flag = PINMUX_I2C_FLAG | PINMUX_GPIO_FLAG,
    },
};
...
static int virt_pctrl_dt_node_to_map(struct pinctrl_dev *pctldev,
...
void virt_pctrl_free_map(struct pinctrl_dev *pctldev, struct pinctrl_map *maps, unsigned num_maps)
{
    int i;
...
int virt_pctrl_get_groups_count(struct pinctrl_dev *pctldev)
{
    struct virtual_pinctrl_desc *vpctrl_desc = pinctrl_dev_get_drvdata(pctldev);
...
const char *virt_pctrl_get_group_name(struct pinctrl_dev *pctldev,
                    unsigned selector)
{
    struct virtual_pinctrl_desc *vpctrl_desc = pinctrl_dev_get_drvdata(pctldev);
    if (!vpctrl_desc) {
...
int virt_pctrl_get_group_pins(struct pinctrl_dev *pctldev, unsigned selector, const unsigned **pins, unsigned *num_pins)
{
    int i = 0;
    struct virtual_pinctrl_desc *vpctrl_desc = pinctrl_dev_get_drvdata(pctldev);
    if (!vpctrl_desc) {
...
static const struct pinctrl_ops virt_pctrl_ops = {
    .dt_node_to_map		= virt_pctrl_dt_node_to_map,
    .dt_free_map		= virt_pctrl_free_map,
    .get_groups_count	= virt_pctrl_get_groups_count,
    .get_group_name		= virt_pctrl_get_group_name,
    .get_group_pins		= virt_pctrl_get_group_pins,
};

int virt_pmx_get_funcs_cnt(struct pinctrl_dev *pctldev)
{
    DEBUG_LOG("Enter!");
...
const char *virt_pmx_get_func_name(struct pinctrl_dev *pctldev,
                    unsigned selector)
{
    DEBUG_LOG("Enter!");
...
int virt_pmx_get_func_groups(struct pinctrl_dev *pctldev,
                unsigned selector,
...
int virt_pmx_set_mux(struct pinctrl_dev *pctldev, unsigned func_selector,
...
int virt_pmx_gpio_request_enable(struct pinctrl_dev *pctldev,
				    ...
				    unsigned offset)
{
    ...
    DEBUG_LOG("Set pin%d as gpio!", offset);
    return 0;
}
...
static const struct pinmux_ops virt_pmx_ops = {
    ...
    .get_function_groups	= virt_pmx_get_func_groups,
    .gpio_request_enable = virt_pmx_gpio_request_enable,
    .set_mux		= virt_pmx_set_mux,
    //.strict			= false, //true,
};



int virt_pconf_set(struct pinctrl_dev *pctldev,
                unsigned pin,
                unsigned long *configs,
...
int virt_pconf_get(struct pinctrl_dev *pctldev,
                unsigned pin,
...
void virt_pconf_dbg_show(struct pinctrl_dev *pctldev,
                    struct seq_file *s,
                    unsigned offset)
{
    int i = 0;
...
void virt_pconff_group_dbg_show(struct pinctrl_dev *pctldev,
                    struct seq_file *s,
                    unsigned selector)
{
...
int virt_pconf_group_set(struct pinctrl_dev *pctldev,
                    unsigned selector,
                    unsigned long *configs,
                    unsigned num_configs)
{
...
int virt_pconf_group_get(struct pinctrl_dev *pctldev,
				     unsigned selector,
				     unsigned long *config)
{
    struct vpctrl_groups *group = NULL;
...
static const struct pinconf_ops virt_pconf_ops = {
    .pin_config_set		= virt_pconf_set,
    .pin_config_group_set = virt_pconf_group_set,
    .pin_config_get = virt_pconf_get,
    .pin_config_group_get = virt_pconf_group_get,
    .pin_config_dbg_show	= virt_pconf_dbg_show,
	.pin_config_group_dbg_show = virt_pconff_group_dbg_show,
};
...
int local_virt_pinctrl_probe(struct platform_device *pdev)
{
    ...
    // - 设置oprt结构体
    //      * pctlops
    g_vpinctrl->pctl_desc.pctlops = &virt_pctrl_ops;
    //      * pmxops
    g_vpinctrl->pctl_desc.pmxops = &virt_pmx_ops;
    //      * confops
    g_vpinctrl->pctl_desc.confops = &virt_pconf_ops;

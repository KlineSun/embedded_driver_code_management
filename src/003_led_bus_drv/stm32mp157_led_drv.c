...
int stm32mp157_led_probe(struct platform_device *pdev)
{
    ...
    DEBUG_LOG("Enter!");
    snprintf(g_led_oprts.dev_name, MAX_LED_DEV_NAME_LEN, "%s", pdev->name);
    ...
    for (i = 0; i < pdev->num_resources; i++) {
        ...
        led_group_pin[led_dev_cnt++] = res->start;
    }

    if (led_dev_cnt <= 0) {
        DEBUG_LOG("get platform device resources failed!");
        return -1;
    }
...
int stm32mp157_led_remove(struct platform_device *pdev)
{
    ...
    DEBUG_LOG("Remove device name: %s, led devices count: %d", pdev->name, led_dev_cnt);
...
/*
static int __init stm32mp157_led_driver_init(void)
{
    DEBUG_LOG("Enter!");
    platform_driver_register(&stm32mp157_led_driver);
    return 0;
}

static void __exit stm32mp157_led_driver_exit(void)
{
    DEBUG_LOG("Enter!");
    platform_driver_unregister(&stm32mp157_led_driver);
}

module_init(stm32mp157_led_driver_init);
module_exit(stm32mp157_led_driver_exit);
*/

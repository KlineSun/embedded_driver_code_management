...
#include "button_irq_thread_drv.h"
...
static void button_tasklet_func(unsigned long priv)
{
    ...
    DEBUG_LOG("Enter tasklet function with KEY%d", btn->btn_num);
}

// 工作队列
static void button_work_function(struct work_struct *wk)
{
    struct local_button *btn = container_of(wk, struct local_button, work);

    if (!btn) {
        DEBUG_LOG("Convert pointer from private data failed!");
        return;
    }
    DEBUG_LOG("Enter workqueue function with KEY%d", btn->btn_num);
}

static irqreturn_t button_irq_bottom_func(int irq, void *priv_data)
{
    ...
    DEBUG_LOG("Enter irq bottom function with KEY%d", btn->btn_num);
    ev.btn = btn;
...
// 中断上半部，不会被其他中断
static irqreturn_t button_irq_top_func(int irqno, void *priv_data)
{
	struct local_button *btn = priv_data;
...
int stm32mp157_button_probe(struct platform_device *pdev)
{
    ...
    for_each_child_of_node(pn, chid) {
        ...
        // request interrupt
        // ret = request_irq(g_btn_list[idx].irq, button_irq_top_func, IRQF_TRIGGER_FALLING, g_btn_list[idx].label, &g_btn_list[idx]);
        // if (ret) {
        //     DEBUG_LOG("request irq for %s failed: %d", g_btn_list[idx].label, ret);
        //     continue;
        // }
        ret = request_threaded_irq(g_btn_list[idx].irq, button_irq_top_func,
                        button_irq_bottom_func, IRQF_TRIGGER_FALLING,
                         g_btn_list[idx].label, &g_btn_list[idx]);
        if (ret) {
            DEBUG_LOG("request irq for %s failed: %d", g_btn_list[idx].label, ret);
            continue;
        }
        ...
        // init workqueue
        INIT_WORK(&(g_btn_list[idx].work), button_work_function);
...
int stm32mp157_button_remove(struct platform_device *pdev)
{
    ...
    for (i = 0; i < btn_irq_cnt; i++) {
        ...
        if (cancel_work_sync(&(g_btn_list[i].work))) {
            flush_work(&(g_btn_list[i].work));
        }
        // destroy_workqueue(&(g_btn_list[i].work));
    }
